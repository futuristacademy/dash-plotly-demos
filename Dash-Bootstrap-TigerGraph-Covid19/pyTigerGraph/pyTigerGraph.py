...
class TigerGraphConnection(object):
    ...
    def __init__(self,dev=False, host="http://localhost", graphname="MyGraph", username="tigergraph", password="tigergraph", restppPort="9000", gsPort="14240", version="3.0.0", apiToken="", useCert=True, certPath=None):
    ...
    def _req(self, method, url, authMode="token", headers=None, data=None, resKey="results", skipCheck=False, params=None):
        ...
        if self.debug:
            print(res[resKey])
        # print("Request End  Time {}".format(str(datetime.timedelta(seconds=(time.time()-start_time)))))
        # print("--------------------------------------------------------")
        return res[resKey]
    ...
    #     if os.path.exists(self.jarLocation + '/gsql_client.jar'):
    ...
    #     """
        ...
        # print("Query Time--- {} seconds ---".format(time.time() - start_time))
        # try:
